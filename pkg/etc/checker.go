...
func Check(config Config) (err error) {
	...
	if config.Vul.CacheDir == "" {
		err = errors.New("vul cache dir must not be blank")
		return
	}

	if config.Vul.ReportsDir == "" {
		err = errors.New("vul reports dir must not be blank")
		return
	}

	if err = ensureDirExists(config.Vul.CacheDir, "vul cache dir"); err != nil {
		return
	}

	if err = ensureDirExists(config.Vul.ReportsDir, "vul reports dir"); err != nil {
		return
	}
