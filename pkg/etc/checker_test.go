...
func TestCheck(t *testing.T) {
	...
	t.Run("Should return error when vul cache dir is blank", func(t *testing.T) {
		tempDir := t.TempDir()
		reportsDir := path.Join(tempDir, "reports")

		err := Check(Config{Vul: Vul{
			CacheDir:   "",
			ReportsDir: reportsDir,
		}})

		assert.EqualError(t, err, "vul cache dir must not be blank")
	})

	t.Run("Should return error when vul reports dir is blank", func(t *testing.T) {
		tempDir := t.TempDir()
		...
		assert.EqualError(t, err, "vul reports dir must not be blank")
	})

	t.Run("Should create vul directories", func(t *testing.T) {
		tempDir := t.TempDir()
	...
	t.Run("Should skip creating vul directories", func(t *testing.T) {
		tempDir := t.TempDir()
