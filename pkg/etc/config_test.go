...
func TestGetConfig(t *testing.T) {
	testCases := []struct {
		...
		{
			name: "Should enable Vul debug mode when log level is set to debug",
			envs: Envs{
			...
			expectedConfig: Config{
				...
				Vul: Vul{
					DebugMode:      true,
					CacheDir:       "/home/scanner/.cache/vul",
				...
				RedisStore: RedisStore{
					Namespace:  "harbor.scanner.vul:data-store",
					ScanJobTTL: parseDuration(t, "1h"),
				},
				JobQueue: JobQueue{
					Namespace:         "harbor.scanner.vul:job-queue",
					WorkerConcurrency: 1,
				},
			},
		},
		...
		},
		{
			...
			envs: Envs{
				...
				"SCANNER_VUL_CACHE_DIR":       "/home/scanner/vul-cache",
				"SCANNER_VUL_REPORTS_DIR":     "/home/scanner/vul-reports",
				"SCANNER_VUL_DEBUG_MODE":      "true",
				"SCANNER_VUL_VULN_TYPE":       "os,library",
				"SCANNER_VUL_SECURITY_CHECKS": "vuln",
				"SCANNER_VUL_SEVERITY":        "CRITICAL",
				"SCANNER_VUL_IGNORE_UNFIXED":  "true",
				"SCANNER_VUL_INSECURE":        "true",
				"SCANNER_VUL_SKIP_UPDATE":     "true",
				"SCANNER_VUL_OFFLINE_SCAN":    "true",
				"SCANNER_VUL_GITHUB_TOKEN":    "<GITHUB_TOKEN>",
				"SCANNER_VUL_TIMEOUT":         "15m30s",
			...
			expectedConfig: Config{
				...
				Vul: Vul{
					CacheDir:       "/home/scanner/vul-cache",
					ReportsDir:     "/home/scanner/vul-reports",
					DebugMode:      true,
					VulnType:       "os,library",
					SecurityChecks: "vuln",
					Severity:       "CRITICAL",
...
func TestGetScannerMetadata(t *testing.T) {
	testCases := []struct {
	...
	}{
		{
			...
			envs:            Envs{"VUL_VERSION": "0.1.6"},
			expectedScanner: harbor.Scanner{Name: "Vul", Vendor: "KhulnaSoft Security", Version: "0.1.6"},
		},
		{
			name:            "Should return unknown version when it is not set via env",
			expectedScanner: harbor.Scanner{Name: "Vul", Vendor: "KhulnaSoft Security", Version: "Unknown"},
		},
	}
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
