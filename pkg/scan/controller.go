...
func (c *controller) scan(scanJobID string, req harbor.ScanRequest) (err error) {
	...
	scanReport, err := c.wrapper.Scan(vul.ImageRef{Name: imageRef, Auth: auth, Insecure: insecureRegistry})
	if err != nil {
		return xerrors.Errorf("running vul wrapper: %v", err)
	}
...
func (c *controller) ToRegistryAuth(authorization string) (auth vul.RegistryAuth, err error) {
	if authorization == "" {
		return vul.NoAuth{}, nil
	}
...
func (c *controller) decodeBasicAuth(value string) (auth vul.RegistryAuth, err error) {
