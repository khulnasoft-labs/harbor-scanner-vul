...
func NewWrapper(config etc.Vul, ambassador ext.Ambassador) Wrapper {
	return &wrapper{
		config:     config,
...
func (w *wrapper) Scan(imageRef ImageRef) ([]Vulnerability, error) {
	...
	if err != nil {
		...
		}).Error("Running vul failed")
		return nil, fmt.Errorf("running vul: %v: %v", err, string(stdout))
	}
...
func (w *wrapper) prepareScanCmd(imageRef ImageRef, outputFile string) (*exec.Cmd, error) {
	...
	name, err := w.ambassador.LookPath(vulCmd)
	if err != nil {
		return nil, err
	}
	...
	cmd.Env = append(cmd.Env, fmt.Sprintf("VUL_TIMEOUT=%s", w.config.Timeout.String()))

	switch a := imageRef.Auth.(type) {
	case NoAuth:
	case BasicAuth:
		cmd.Env = append(cmd.Env,
			fmt.Sprintf("VUL_USERNAME=%s", a.Username),
			fmt.Sprintf("VUL_PASSWORD=%s", a.Password))
	case BearerAuth:
		cmd.Env = append(cmd.Env,
			fmt.Sprintf("VUL_REGISTRY_TOKEN=%s", a.Token))
	default:
		return nil, fmt.Errorf("invalid auth type %T", a)
	}

	if imageRef.Insecure {
		cmd.Env = append(cmd.Env, "VUL_NON_SSL=true")
	}
	...
	if w.config.Insecure {
		cmd.Env = append(cmd.Env, "VUL_INSECURE=true")
	}
...
func (w *wrapper) GetVersion() (VersionInfo, error) {
	...
	if err != nil {
		return VersionInfo{}, fmt.Errorf("failed preparing vul version command: %w", err)
	}

	versionOutput, err := w.ambassador.RunCmd(cmd)
	if err != nil {
		return VersionInfo{}, fmt.Errorf("failed running vul version command: %w: %v", err, string(versionOutput))
	}

	var vi VersionInfo
	err = json.Unmarshal(versionOutput, &vi)
	if err != nil {
		return VersionInfo{}, fmt.Errorf("failed parsing vul version output: %w", err)
	}
