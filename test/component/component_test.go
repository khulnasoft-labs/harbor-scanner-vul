...
var (
	vulScanner = harbor.Scanner{Name: "Vul", Vendor: "KhulnaSoft Security", Version: "0.44.0"}
)
...
const (
	adapterImage = "khulnasoft/harbor-scanner-vul:dev"
...
func TestComponent(t *testing.T) {
	...
	for _, cc := range testCases {
		...
		t.Run(fmt.Sprintf("Should scan %s", imageRef), func(t *testing.T) {
			...
			assert.Equal(t, vulScanner, report.Scanner)
			// TODO Adding asserts on CVEs is tricky as we do not have any control over upstream vulnerabilities database used by Vul.
